...
async function fetchTotalLearners() {
  try {
    const response = await fetch('http://localhost:3000/api/learners/count');
...
async function removeLearner(id) {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/learners/${id}`, { method: 'DELETE' });
    if (response.ok) {
...
async function fetchLearnerDetails(id) {
  try {
    const allLearners = await (await fetch('http://localhost:3000/api/learners')).json();
    ...
    const response = await fetch(`http://localhost:3000/api/learners/${id}/attendance`);
